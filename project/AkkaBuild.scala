...
object AkkaBuild {
  ...
  lazy val defaultSettings: Seq[Setting[_]] = Def.settings(
    ...
    scalacOptions in Compile ++=
      CrossJava.targetJdkScalacOptions(CrossJava.Keys.targetSystemJdk.value, CrossJava.Keys.fullJavaHomes.value),
    scalacOptions in Compile ++= (if (allWarnings) Seq("-deprecation") else Nil),
    scalacOptions in Test := (scalacOptions in Test).value.filterNot(opt =>
      opt == "-Xlog-reflective-calls" || opt.contains("genjavadoc")),
    javacOptions in compile ++= DefaultJavacOptions ++
      CrossJava.targetJdkJavacOptions(CrossJava.Keys.targetSystemJdk.value, CrossJava.Keys.fullJavaHomes.value),
