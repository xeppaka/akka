...
object CrossJava {
  ...
  private def selectOptions(targetSystemJdk: Boolean, fullJavaHomes: Map[String, File], jdk8options: Seq[String], jdk11options: File => Seq[String]): Seq[String] =
    if (targetSystemJdk)
      ...
      case None =>
        throw new MessageOnlyException("A JDK 8 installation was not found, but is required to build Akka. To target your system JDK, `set every akka.CrossJava.Keys.targetSystemJdk := true`. Resulting artifacts may not work on JDK 8")
    }
