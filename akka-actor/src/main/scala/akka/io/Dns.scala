...
class DnsExt private[akka] (val system: ExtendedActorSystem, resolverName: String, managerName: String)
    ...
    asyncDns.computeIfAbsent(
      ...
      new JFunction[String, ActorRef] {
        override def apply(r: String): ActorRef = {
          ...
          val provider = system.dynamicAccess.getClassFor[DnsProvider](settings.ProviderObjectName).get.getDeclaredConstructor().newInstance()
  ...
  val provider: DnsProvider =
    system.dynamicAccess.getClassFor[DnsProvider](Settings.ProviderObjectName).get.getDeclaredConstructor().newInstance()
